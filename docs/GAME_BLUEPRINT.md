...
| 2026-09-12 | เพิ่ม disk space + auto-refresh ให้ dashboard พบและแก้ race condition จริงในชุด test ทั้งหมด (module-level DB singleton เปิดไฟล์จริงชนกันข้าม vitest worker) — ดู GAME_WIKI.md §5.7.2 | `docs/archive/2026-09-11-telemetry-error-logging.md` |
| 2026-09-12 | ตัดกราฟดิสก์ตามเวลาออก, เปลี่ยน card pick rate เป็นตารางแยกทีละใบการ์ด, รัน `/impeccable layout` แก้ grid/emoji/unbounded-table — ดู GAME_WIKI.md §5.7.3-5.7.4 | `docs/archive/2026-09-11-telemetry-error-logging.md` |
...
| 2026-09-13 | User report: deploy (`pm2 restart`) ทำให้คนกำลังเล่นอยู่ disconnect แบบไม่รู้สาเหตุ — เพิ่ม `SERVER_SHUTDOWN_WARNING` broadcast 30 วิก่อน restart (ไม่ใช่ zero-downtime, game state ยังอยู่ memory ล้วนๆ) `ServerMessage` ใหม่ + `broadcastToAll()`/`POST /api/admin/broadcast-shutdown-warning` ใหม่ใน `server.ts`, client banner ใหม่ใน `main.ts` — ⚠️ deploy รอบแรกเปิด endpoint นี้แบบไม่มี auth เพราะเข้าใจผิดเรื่อง nginx proxy scope แก้ทันทีด้วย `ADMIN_SECRET` — ดู GAME_WIKI.md §5.5 | `docs/archive/2026-09-13-deploy-shutdown-warning.md` |
>
...
| 28 | ✅ **แก้แล้ว 2026-09-12** — ~~ไม่มี `vitest.config.ts` — test count เพี้ยนจาก git worktree อื่นที่ทำงานขนานกัน~~ (§4.7, พบจาก session `card-list-documentation` ตรวจสอบ archive spec #27) | เพิ่ม `vitest.config.ts` (`exclude: [...configDefaults.exclude, '.claude/**']`) กัน sweep เจอ `*.test.ts` ของ `.claude/worktrees/*` — ยืนยัน 93/93 คงที่ (เดิมรายงานผิดเป็น 293/293 ในสอง commit ก่อนหน้า) | **S** | เสร็จแล้ว |
