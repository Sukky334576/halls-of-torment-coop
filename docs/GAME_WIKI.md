...
| 2026-09-12 | ทำ `docs/card-list.md` ใหม่ (quick-reference ตาราง 70 ใบจาก `TRAIT_POOL`) ระหว่างทางพบ **risk ใหม่ #27**: การ์ด 5 ใบที่ `description` พูดถึง mechanic/ตัวเลขที่ไม่ตรงกับสิ่งที่ `apply()`/โค้ดจริงทำ (บางใบ mechanic ไม่มีอยู่จริงเลย) — ไล่หาต่อจนสุดสายด้วยการ grep ทุกจุดที่ field ถูกอ่านจริงใน `GameRoom.ts`/`ServerPlayer.ts` (session `card-list-documentation`) | คำสั่ง user: สำรวจ repo สร้าง card-list.md แล้ว "ไล่หาต่อ" ว่าตัวเลขที่หายไปอยู่ไฟล์ไหน |
| 2026-09-12 | แก้ risk #27 (description กับ apply()/โค้ดจริงของ 5 การ์ดไม่ตรงกัน) — 2 ใบแก้โค้ด (`magnet_1` เพิ่ม expMultiplier, `gambler_fortune_greed` ให้ coin-drop chance scale ตาม rank), 2 ใบแก้ description (`commando_ap_rounds`, `cowboy_quick_draw`), 1 ใบไม่ต้องแก้ (`gambler_royal_flush` เป็น pattern ปกติของระบบ ไม่ใช่ bug) | `docs/archive/2026-09-12-card-description-mismatch-fix.md` |
