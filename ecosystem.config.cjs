...
// here — they live in a gitignored `.env.server` file in this same directory (see
// .env.server.example) and are loaded by src/server/server.ts itself via `dotenv`. This file
// only carries non-secret process config, so it's safe to commit.
//
